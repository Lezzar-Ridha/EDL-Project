...
onBeforeMount(() => {
    axios.get('https://127.0.0.1:8000/users/type/viceDean').then(res => {
      vd.value = res.data
...
function checked(vd){
    form.id = vd._id
    form.firstName = vd.firstName
    form.lastName = vd.lastName
    form.email = vd.email
...
<template>
  ...
  <table>
    ...
    <tbody>
      <tr v-for="client in vd.value" :key="client.id">
